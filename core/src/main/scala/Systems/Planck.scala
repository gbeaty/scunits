...
trait Planck {
  ...
  val intensity = (power / area).label("planck intensity","MT-3") // 7s
  // val a: power.dims = 1
  // val b: area.dims = 1
  // val angularFrequency = (coef / time).label("planck frequency","T-1") 17s
  // val pressure = (area / force).label("planck pressure","L-1MT-2") 17s
  // val current = (charge / time).label("planck current","QT-1") 17s
  // val voltage = (power / current).label("planck voltage","L2MT-2Q-1") 17s
  // val impedance = (voltage / current).label("planck impedance","L2MT-1Q-2") // 18s
}
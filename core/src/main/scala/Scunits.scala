...
case class UnitM[D <: Dims](name: String, symbol: String, toSI: Double => Double) {
  protected[scunits] def construct(v: Double) = Measure[D](toSI(v))
  def apply(v: Double) = construct(v)
}
object UnitM {
  def apply[D <: Dims](name: String, symbol: String, mult: Double = 1.0, offset: Double = 0.0): UnitM[D] =
    UnitM[D](name, symbol, (i: Double) => i * mult + offset)
}

class Prefix(val name: String, val symbol: String, val mult: Double) {
  def apply[D <: Dims](u: UnitM[D]) = UnitM[D](name + u.name, symbol + u.symbol, (i: Double) => u.toSI(mult * i))
  def apply[D <: Dims](u: UnitM[D], v: Double) = u.construct(mult * v)
}
